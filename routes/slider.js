...
router.post("/slider/slides", middlewares.isLoggedIn, async (req, res) => {
	if (req.body) {
		...
		if (req.files) {
			...
			if (!imgPath) {
				res.status(501).json({ err: 'img is too large' });
				return;
			}
		...
		var data = req.body;
		data.img = link;
		data.show_img_first === "on"
			? (data.show_img_first = true)
			: (data.show_img_first = false);
...
router.put("/slider", middlewares.isLoggedIn, async (req, res) => {
	if (req.body) {
		...
		if (
			req.files &&
			deleteFile(getImgName(JSON.parse(oldData).img), "slides")
		) {
			newData.img = link;
		} else {
		...
		Slider.find({}, (err, sliders) => {
			...
			Slider.findById(sliders[0]._id, (err, slider) => {
				...
				var slideIndex = slider.slides.findIndex(
					(o) =>
						o.title === JSON.parse(oldData).title &&
						o.img === JSON.parse(oldData).img
				);
				...
				slider.save((err, slide) => {
					...
					res
						.status(200)
						.json({ status: true, slide: slider.slides[slideIndex] });
				});
			});
		});
	} else {
	...
	if (req.body) {
		Slider.find({}, (err, sliders) => {
			...
			Slider.findById(sliders[0]._id, (err, slider) => {
				...
				var slideIndex = slider.slides.findIndex(
					(o) => o.title === req.body.title && o.img === req.body.img
				);
